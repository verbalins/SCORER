...
freqchart <- function(.data, objectives = names(attr(.data, "objectives")), rank = 1, n = 10) {
  filtered <- .data %>%
    ...
    dplyr::select_at(dplyr::vars(dplyr::starts_with("imp")), function(x) stringr::str_replace(x, "^imp_", ""))
  ...
  frequencies %>%
    utils::head(n) %>%
    ggplot2::ggplot(ggplot2::aes(x = stats::reorder(key, -freq),
                                 y = freq,
                                 #label = signif(freq*100, 4))) +
                                 label = scales::percent(freq, accuracy = 0.1, suffix = "%"))) +
    ggplot2::geom_col() +
    ggtext::geom_richtext(mapping = ggplot2::aes(angle=90),
                          show.legend = FALSE,
                          position = ggplot2::position_stack(vjust = 0.5),
                          label.padding = ggplot2::unit(c(0.25, 0.75, 0.25, 0.25), "lines")) +
    ggplot2::labs(y = "Frequency",
                  x = "Parameter") +
    ggplot2::scale_y_continuous(limits = c(0, 1), labels = scales::percent, expand = ggplot2::expansion(mult = c(0, 0))) +
...
plotPareto <- function(.data, objectives = names(attr(.data, "objectives")), interactive = FALSE) {
  ...
  chart <- .data %>%
    dplyr::distinct_at(dplyr::vars(all_of(objectives),
                                   ...
                                   Rank),
                       .keep_all = T) %>% # Always unique solutions to a SCORE problem.
    ggplot2::ggplot(ggplot2::aes_string(x = dplyr::if_else("minImp" %in% objectives, "minImp", objectives[1]),
                                        y = dplyr::if_else("maxOut" %in% objectives, "maxOut", objectives[2]), color = "Rank")) +
    ggplot2::geom_point() +
    ggplot2::geom_line(data = .data %>% dplyr::filter(Rank == 1), color = "red") +
  ...
  if (interactive) {
    p <- .data %>%
      plotly::plot_ly(x = ~minImp, y = ~maxOut, color = ~Rank, type = "scattergl", mode = "markers",
                      hoverinfo = "text",
                      text = ~paste('</br> Improvements: ', minImp,
                                    '</br> Out: ', round(maxOut),
                                    '</br> Rank: ', Rank),
                      name = "Rank 1-5")
    if (length(objectives) == 2) {
      p <- p %>% plotly::add_trace(data = .data %>%
                                     dplyr::filter(Rank == 1) %>%
                                     dplyr::select(-Iteration, minImp, maxOut) %>%
                                     dplyr::arrange(minImp),
                                   type = "scattergl",
                                   mode = "lines",
                                   name = "Pareto Front",
                                   color = NULL)
    } else if(length(objectives) == 3) {
      p <- p %>% plotly::add_trace(z = ~minLT,
                                   type = "scatter3d",
                                   size = 2,
                                   text = ~paste('</br> Improvements: ', minImp,
                                                 '</br> Out: ', round(maxOut),
...
exportPlot <- function(.plot = ggplot2::last_plot(), filename, path = getwd(), device = "pdf", width = 15, height = 7, ...) {
  if (device == "pdf" || device == "png") {
    ggplot2::ggsave(paste0(filename, ".", device),
                    path = path,
                    plot = .plot,
                    device = device,
                    family = "sans",
    ...
    tikzDevice::tikz(file = paste0(path, filename, ".tex"),
