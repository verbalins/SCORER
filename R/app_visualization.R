  ...
  shiny::moduleServer(id,
    function(input, output, session) {
      ...
      output$x <- shiny::renderUI({
        ns <- session$ns
        shiny::selectInput(ns("x"), "X:", dim_list(),
                           selectize = FALSE,
                           selected = dim_list()$Objectives[[1]])
      })

      output$y <- shiny::renderUI({
        ns <- session$ns
        shiny::selectInput(ns("y"), "Y:", dim_list(),
                           selectize = FALSE,
                           selected = dim_list()$Objectives[[2]])
      })

      output$z <- shiny::renderUI({
        ns <- session$ns
        shiny::selectInput(ns("z"), "Z:", dim_list(),
                           selectize = FALSE,
                           selected = dim_list()$Objectives[[3]])
      })

      output$color <- shiny::renderUI({
        ns <- session$ns
        choice <- grep("Rank|Distance|Cluster",
                       colnames(r$filtered_data),
                       value = TRUE)
        shiny::selectInput(ns("color"), "Color:",
                           choice, selectize = FALSE)
      })
      ...
      shiny::observeEvent(plotly::event_data("plotly_restyle", source = "pcoords"), {
        ...
        dimension_name <- unlist(c(dim_list()$Objectives,
                                   dim_list()$Inputs))[[dimension + 1]]
        ...
        for (i in names(ranges$data)) {
          ...
          for (j in seq_along(range_)) {
            ...
            if (length(rng) == 0) # When deselecting
              ...
              keep_var <- keep_var | dplyr::between(r$filtered_data[[i]],
                                                    min(rng),
                                                    max(rng))
            }
          }
      ...
      # they are visible on the web page.
