...
mod_rule_fpm_server <- function(id, rval) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$fpmapplyrules <- shiny::renderUI({
      if (input$pointsel == "Reference point") {
        shiny::tagList(shiny::actionButton(ns("fpmrulebutton"),
                                           "Create Rules"))
      } else {
        shiny::tagList(shiny::validate(
          shiny::need(
            nrow(rval$df_selected()$unsel) != 0,
    ...
    shiny::observeEvent(input$refAssign, {
      ...
      selected <-
        SCORER::assign_reference_point(
          shiny::isolate(rval$filtered_data),
          reference_point,
    ...
    shiny::observeEvent(input$fpmrulebutton, {
      ...
      if (grepl("Reference", input$pointsel)) {
        ...
        rval$selected_data <-
          rval$df_selected()$sel$Iteration
    ...
    output$fpm_plot <- plotly::renderPlotly({
      shiny::validate(shiny::need(
        !is.null(rval$plotdims()$x),
